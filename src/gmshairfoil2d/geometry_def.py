...
class CurveLoop:
    def __init__(self, startTag, endTag):
        ...
        self.curveTags = np.linspace(self.startTag, self.endTag, self.nbLine).astype(
            int
        )
...
class Circle:
    def __init__(self, xc, yc, zc, r):
        ...
        # radius of the disk
        self.r = r
        # Disk object are of dim 1
        self.dim = 1
        # create the gmsh object and store the tag of the geometric object
        self.tag = gmsh.model.occ.addCircle(self.xc, self.yc, self.zc, self.r)
